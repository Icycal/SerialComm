...
// Dialog Data
	...
	CHyperLink	m_ctrlHyperLinkWWWBLFZ;
	//}}AFX_DATA
...
void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_Icy21, m_ctrlHyperLinkWWWBLFZ);
	//}}AFX_DATA_MAP
}
BOOL CAboutDlg::OnInitDialog() 
{
	...
	m_ctrlHyperLinkWWWBLFZ.SetURL(_T("http://bbs.21ic.com/home.php?mod=space&uid=993588"));
	m_ctrlHyperLinkWWWBLFZ.SetUnderline(TRUE);
	m_ctrlHyperLinkWWWBLFZ.SetLinkCursor(AfxGetApp()->LoadCursor(IDC_CURSOR_HAND));
...
void CSCOMMDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_BLFZ21, m_ctrlHyperLinkWWW);
	DDX_Control(pDX, IDC_STATIC_BING, m_ctrlBLFZ);
	...
	DDX_Control(pDX, IDC_STATIC_ICYCAL, m_ctrlHyperLink2);
...
BEGIN_MESSAGE_MAP(CSCOMMDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_HEXRECIEVE, OnCheckHexrecieve)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
/*
...
BOOL CSCOMMDlg::OnInitDialog()//控件的初始化都放在这个里面的
{
	...
	//m_ctrlHyperLink2.SetURL(_T("mailto:IcycalGu@163.com"));
	m_ctrlHyperLink2.SetUnderline(TRUE);
	m_ctrlHyperLink2.SetLinkCursor(AfxGetApp()->LoadCursor(IDC_CURSOR_HAND));//

	m_ctrlHyperLinkWWW.SetURL(_T("http://bbs.21ic.com/space-username-%E5%86%B0%E9%9B%B6%E5%88%86%E5%AD%90.html"));
...
//	m_ctrlIconDatareceive.SetIcon(m_hIconOff);
    ...
    { 
        ...
        if(pFunc!=NULL) 
        { 
            pFunc(m_hWnd, 0, 240, 2);    //2:LWA_ALPHA 
        } 
        FreeLibrary(hInst); 
        hInst=NULL; 
    }
...
void CSCOMMDlg::OnButtonSavedata() 
{
	...
	CFile m_rFile;
	LPCSTR	lpszPath =m_strCurPath;// "D:\\comdata";
	...
	for(int j=0;j<100;j++)
	{
		...
		if( (access( buf, 0 )) == -1 )//检查文件是否存在 存在返回0 否则返回-1
			break;
	}
...
void CSCOMMDlg::OnCheckHexrecieve() 
{
	// TODO: Add your control notification handler code here
